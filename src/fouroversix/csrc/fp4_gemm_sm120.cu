...
namespace fouroversix
{
    ...
    torch::Tensor gemm_fp4fp4_accum_fp32_out_bf16_tnt_sm120(torch::Tensor const &A, torch::Tensor const &B, torch::Tensor const &A_sf, torch::Tensor const &B_sf, torch::Tensor const &alpha)
    {
    ...
    // Adapted from example 72b
    torch::Tensor gemm_fp4fp4_accum_fp32_out_fp16_tnt_sm120(torch::Tensor const &A, torch::Tensor const &B, torch::Tensor const &A_sf, torch::Tensor const &B_sf, torch::Tensor const &alpha)
    {
        using ElementA = cutlass::nv_float4_t<cutlass::float_e2m1_t>;
        using ElementB = cutlass::nv_float4_t<cutlass::float_e2m1_t>;
    ...
    TORCH_LIBRARY_IMPL(fouroversix, CUDA, m)
    {
        ...
        m.impl("gemm_nvfp4nvfp4_accum_fp32_out_bf16_tnt_sm120",
                &gemm_fp4fp4_accum_fp32_out_bf16_tnt_sm120);
        m.impl("gemm_nvfp4nvfp4_accum_fp32_out_fp16_tnt_sm120",
                &gemm_fp4fp4_accum_fp32_out_fp16_tnt_sm120);
    }
}